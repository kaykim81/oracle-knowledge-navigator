...
**Experiment 3 — the OOV recall-rescue regime (`evals/oov_slice.py`, standalone analysis).** Built the regime where dense *genuinely fails on recall*: minimal-context lookups of ultra-rare exact identifiers (df=2) the embedder can't localize. On n=8: `vector_only` collapses (MRR **0.275**, misses **5/8**), `hybrid_rerank` fully rescues to **1.000** — hybrid winning outright, the case the clean eval couldn't surface. Two mechanism findings: RRF is a poor OOV fusion (0.497 — it penalizes single-leg finds; `reference1` dropped from top-10 despite being BM25 rank-1), and the regex adaptive router misses acronym OOV (`CCSP`) → motivates an IDF-based router. Caveats: light query context rescues vector (bare `XCC` missed, "XCC flexfield code" found), and the regime is scarce on this clean corpus (~3 misses per 16 ultra-rare probed). Synthesis: hybrid's value is regime-specific and the fusion mechanism decides whether it's captured — `hybrid_rerank` (sparse+dense pool → rerank) wins all three regimes.

**IDF-based adaptive router (`HYBRID_ADAPTIVE_SIGNAL=regex|idf|both`, `HYBRID_LEX_DF_MAX`).** To fix the regex router's acronym blind spot (Exp 3: `CCSP` missed), added a rarity signal routing on the rarest content token's document frequency (`db.token_doc_freq` / `db.content_tokens`, smoke-tested). It *does* fix OOV (idf/both rescue `CCSP`/`reference1` to rank 1) **but is a noisy router signal**: it regressed adversarial (`hybrid` 0.539 → 0.486) via two confounds — off-domain lure words (`financial` df=1 *in OCI*; fixed by switching to *global* IDF) and ordinary-but-rare words (`isolate` df=4, `hosting` df=2; unfixable by threshold — they overlap real identifiers at df 2–3). Verdict: regex stays the precise default; idf/both are opt-in for OOV-heavy workloads; a clean router needs orthography (all-caps/alphanumeric shape) + rarity, or a learned classifier. Defaults unchanged (`rrf`/`regex`). Full analysis in `TEST_LOG.md`.

**Experiment 1 — multi-doc set-recall via pooled judgments (`evals/multidoc_eval.py`).** Closed the "metric" leg of the diagnosis (cause #2). Free keyword/section gold is too coarse (32–141 chunks), so used TREC-style pooled LLM judgments: pool top-10 of all 4 modes per (query, product) over single+cross (n=45), Sonnet grades each pooled passage 0–3, gold = judged ≥2 (avg 4.7/unit), score set-recall@10 + graded nDCG@10. **Result: `hybrid` (RRF) beats *both* legs on set-recall (0.683 vs vector 0.566 vs keyword 0.429), and `hybrid_rerank` wins (0.791 / nDCG 0.701)** — the union benefit that was *invisible* on single-target recall@1/MRR (where RRF looked strictly worse via dilution). The metric, not the retriever, hid hybrid; set-recall is also the RAG-relevant metric (the LLM needs complete context). ~$0.6. Full writeup in `TEST_LOG.md`. **Hybrid's value depends on two axes — regime *and* metric.**

**Promoted adaptive fusion to the production default + redeployed (2026-06-01).** A/B on the *shipped* `hybrid_rerank` showed the candidate pool's fusion is not moot: pool=hybrid+rrf 0.572 → pool=hybrid+adaptive(sem=1.0) **0.595** strict-all (cross recovered 0.475→0.516), ≈ the tuned pool=vector 0.598 — adaptive routes semantic-query pools to pure vector while keeping BM25 for identifier queries. Flipped two defaults (`HYBRID_FUSION=adaptive`, `HYBRID_ALPHA_SEMANTIC=1.0`); kept `signal=regex`/`df_max=5` (idf/`both` immaterial-to-worse for the shipped mode + the adversarial lure confound). Rebuilt + redeployed the 3 MCP servers (runtime verified: fusion=adaptive, sem=1.0, regex, pool=hybrid). Scorecard `192424Z`. Largely retires the earlier "shipped the weaker A/B arm" caveat. (My "reranker makes the pool moot" prediction was wrong — the A/B corrected it.) Full A/B in `TEST_LOG.md`. **Confirmed on the OOV slice:** hybrid_rerank recall@10 = 1.000 (pool=hybrid) vs 0.375 (pool=vector) — the vector pool's +0.005 on the clean bar is wiped out by missing 5/8 OOV tokens, so pool=hybrid is decisively right; `regex` suffices (the reranker recovers OOV from the union pool, so `signal=both` is redundant for the shipped mode).
