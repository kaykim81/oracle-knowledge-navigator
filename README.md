...
End-to-end (LLM-judged, current config — scorecard `20260601T200537Z`): `hybrid_rerank` tops answer quality **overall (3.98 vs hybrid 3.91 / vector 3.88, out of 5; groundedness 4.13)** and in 3 of 4 categories (single 4.40 / cross 3.13 / exact_term 4.24); on adversarial the adaptive `hybrid` mode edges it (4.22 vs 4.16). The margins are now *small* — adaptive fusion lifted the standalone `hybrid` from 3.66 (old RRF) to 3.91, so on answer quality the modes nearly converge, with the reranker still best overall and on the hardest regimes (cross is weakest, ~3.1 across all modes). Routing 93% (100% single/cross; 80% adversarial-hedge).
